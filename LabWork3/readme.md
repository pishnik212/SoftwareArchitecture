...
#### YAGNI (You Aren't Gonna Need It): Код не содержит ненужных функций или сложных структур. Он реализует только те функции, которые необходимы в данный момент, без добавления избыточного функционала.

#### DRY (Don't Repeat Yourself): В серверном коде не повторяются определения моделей и маршрутов. Все повторяющиеся действия (например, добавление в базу данных) обрабатываются с помощью функций, что уменьшает дублирование кода.
